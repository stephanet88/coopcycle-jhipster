...
import com.coopcycle.java.service.CartService;
...
/**
 * REST controller for managing {@link com.coopcycle.java.domain.Cart}.
 */
@RestController
@RequestMapping("/api")
public class CartResource {
     ...
     */
    @GetMapping("/carts")
    public Mono<List<CartDTO>> getAllCarts(@RequestParam(required = false) String filter) {
        if ("order-is-null".equals(filter)) {
            log.debug("REST request to get all Carts where order is null");
            return cartService.findAllWhereOrderIsNull().collectList();
        }
        if ("paymentoption-is-null".equals(filter)) {
            log.debug("REST request to get all Carts where paymentOption is null");
            return cartService.findAllWherePaymentOptionIsNull().collectList();
        }
